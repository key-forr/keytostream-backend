		...
		s = a => {
			a.forEach(u => {
				u.includes('Uint') && (e[u.replace('Uint', 'UInt')] = e[u]),
					u.includes('Float64') &&
						(e[u.replace('Float64', 'Double')] = e[u]),
					u.includes('Float32') &&
						(e[u.replace('Float32', 'Float')] = e[u])
			})
		}
	n.forEach((a, u) => {
		a.startsWith('read') &&
		...
		h = class e extends Uint8Array {
			...
			toString(r = 'utf8', n = 0, i = this.length) {
				if (
					...
					return ''
				if (r === 'utf8' || r === 'utf-8')
					return Yo.decode(this.slice(n, i))
	...
	hn = ae(() => {
		...
		R.toPrecision = function (e, t) {
			...
			return (
				e === void 0
					? ((r = B(i)),
						(n = Oe(i, r <= o.toExpNeg || r >= o.toExpPos)))
					: (ue(e, 1, Le),
						t === void 0 ? (t = o.rounding) : ue(t, 0, 8),
						(i = D(new o(i), e, t)),
						(r = B(i)),
...
${a.backtrace}`,
			...
			return a.map(y =>
				y.errors && y.errors.length > 0
					? (this.loggerRustPanic ??
						this.buildQueryError(y.errors[0]))
					: { data: y }
			)
		throw u && u.length === 1
			? new Error(u[0].error)
	...
	getExternalAdapterError(t) {
		if (t.error_code === Va && this.config.adapter) {
			...
			return (
				Nt(n, 'External error with reported id was not registered'), n
			)
		}
	}
	async metrics(t) {
...
var en =
		...
		r => {
			...
			if (Ci(r))
				...
				let [o, ...s] = r
				;(n = o),
					(i = { values: Xe(s || []), __prismaRawParameters__: !0 })
			} else
				switch (t) {
					case 'sqlite':
					case 'mysql': {
...
function dr(e) {
	...
	for (let n = 0; n < e.rows.length; n++) {
		...
		for (let s = 0; s < i.length; s++)
			o[e.columns[s]] = Me(e.types[s], i[s])
		t.push(o)
	}
	return t
}
