...
export class MailService {
	...
	public async sendVerificationToken(email: string, token: string) {
		...
		return this.sendMail(email, 'Верифікація аккаунта', html)
	}

	public async sendPasswordRestToken(
		email: string,
		token: string,
	...
	public async sendDeactivateToken(
		...
		return this.sendMail(email, 'Деактивація аккаунта', html)
	}
	...
	public async sendAccountDeletion(email: string) {
		...
		return this.sendMail(email, 'Аккаунт видалений', html)
	}
	...
	public async sendEnableTwoFactor(email: string) {
		...
		return this.sendMail(email, 'Забезпечте свою безпеку', html)
	}

	public async sendVerifyChannel(email: string) {
		const html = await render(VerifyChannelTemplate())

		return this.sendMail(email, 'Ваш канал верифіковано', html)
	}
